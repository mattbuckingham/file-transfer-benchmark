...
FILE_SIZE_KB = 1
SOURCE_DIR = Path("./")
FTP_TARGET_DIR = Path("/mnt/storage")
...
def transfer_files():
    print(f"Transferring {NUM_FILES} files to FTP at: {FTP_TARGET_DIR}")
    if not FTP_TARGET_DIR.exists():
    ...
    for file_path in SOURCE_DIR.iterdir():
        shutil.copy2(file_path, FTP_TARGET_DIR)
    end_time = time.time()
